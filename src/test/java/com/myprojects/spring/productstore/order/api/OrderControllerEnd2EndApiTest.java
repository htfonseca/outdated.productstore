...
import com.myprojects.spring.productstore.order.dto.ParcelDto;
...
@SpringBootTest
@DisplayName("Test that Order API")
class OrderControllerEnd2EndApiTest {

  private final RequestFieldsSnippet orderDtoRequestSnippet =
      requestFields(
          fieldWithPath("email").description("Email of the order"),
          fieldWithPath("parcels").description("List of parcel of the order"),
  ...
  private final ResponseFieldsSnippet orderResponseSnippet =
      responseFields(
          fieldWithPath("identifier").description("Order identifier"),
          fieldWithPath("email").description("Email of the order"),
          fieldWithPath("date").description("Date of the order"),
          fieldWithPath("total").description("Total cost of the order"),
          fieldWithPath("parcels").description("List of parcel of the order"),
  ...
  private final ResponseFieldsSnippet orderListResponseSnippet =
      responseFields(
          fieldWithPath("content").description("A list of orders"),
  ...
  @Autowired private OrderRepository productOrderRepository;
  ...
  void setUp() {
    ...
    List<Order> productOrders = new LinkedList<>();
    productOrders.add(
        new Order(
            UUID.randomUUID(),
            "user1@gmail.com",
  ...
  void verifyAndDocumentCreateProductOrderSuccessfully() throws Exception {
    ...
    OrderDto productOrderDto = new OrderDto("newUser@gmail.com", Collections.singletonList(parcel));
    ...
    mockMvc
        ...
        .andExpect(
            header().string(HttpHeaders.LOCATION, startsWith("http://localhost:8080/store/orders")))
        ...
        .andDo(
            document(
                ...
                orderDtoRequestSnippet,
                responseHeaders(
                    headerWithName(HttpHeaders.LOCATION)
                        .description("Location of created order resource.")),
  ...
  void verifyAndDocumentReadProductSuccessfully() throws Exception {
    ...
    Order productOrder = productOrderRepository.findAll().get(0);

    mockMvc
        .perform(
            get("/store/orders/{identifier}", productOrder.getIdentifier())
                .locale(Locale.ENGLISH)
                .accept(MediaTypes.HAL_JSON_UTF8_VALUE))
  ...
  void verifyAndDocumentReadProductsSuccessfully() throws Exception {
    ...
    mockMvc
        ...
        .andDo(
            document(
                "order/document-get-list-order",
                preprocessRequest(prettyPrint()),
                preprocessResponse(prettyPrint()),
                requestParameters(
                    parameterWithName("start")
                        .description("Start date to filter the fetch of the orders."),
                    parameterWithName("end")
                        .description("End date to filter the fetch of the orders.")),
                orderListResponseSnippet,
                simpleLinksSnippet));
  }
}
